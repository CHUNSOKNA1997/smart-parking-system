...
export const generateQRCode = async (bookingData: QRCodeData): Promise<string> => {
  try {
...
export const generateQRCodeBuffer = async (bookingData: QRCodeData): Promise<Buffer> => {
  try {
    const { bookingId, spotId, userId, startTime } = bookingData;
...
// Generate QR code as buffer (for saving to file)
const generateQRCodeBuffer = async (bookingData) => {
  try {
    const { bookingId, spotId, userId, startTime } = bookingData;
